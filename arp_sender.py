...
class ARPSender:
    ...
    def send_arp(self):
        channel = "0"+hex(int(sender_id)).lstrip("0x") if int(sender_id) < 16 else hex(int(sender_id)).lstrip("0x")

        mac_list = mac_list_to_str_list(encode_decode.str_to_hex_list(encrypt_decrypt.encrypt(message)[0]), channel)
